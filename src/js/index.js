(function ($, player) {
	function MusicPlayer(dom) {
		...
		this.progress = player.progress.pro(); //生成一个进度条的实例
	}
	MusicPlayer.prototype = {
		...
		loadMusic:function(index) {	//加载音乐
			...
			if (player.music.status == 'play') {
				...
				this.progress.move(0);  //加上一个0，清空当前的百分比
			}
		...
		dragProgress: function(){
			...
			circle.end = function(per){
				...
				player.music.playTo(cutTime);	// 定位到对应的时间
				player.music.play();		//播放歌曲
